...
class BaseTestCase(TestCase):
    ...
    def AddMockThreadsWithMockMessages(self):
        '''
        Utility function for adding a list of
        mock threads associated with mock messages to database.

        We expect it is sufficient by providing foreign keys in
        the mock data, without linking to corresponding entities.

        However, manipulating foreign keys directly is not
        recommended for sqlalchemy.

        Therefore, it is quite difficult to create relations by
        foreign keys only.
        ...
        for thread in self._mock_threads:
            '''
            Create a copy of the mock thread to prevent the thread from
            being attached to previous session

            Save the User_UserId here because Thread.Copy method does not
        ...
        for msg in self._mock_messages:
            '''
            Create a copy of the mock message to prevent the message from
            being attached to previous session

            Save the User_UserId here because Message.Copy method does not
            manipulate foreign keys
            '''
